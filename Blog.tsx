    ...
    <div className="section" style={{ minHeight: '100vh', paddingTop: '120px' }}>
      <div className="container">
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          <p className="section-subtitle text-gold uppercase tracking-widest text-sm mb-1">MY WRITINGS</p>
          <h2 className="section-title text-white font-playfair text-5xl">The Blog</h2>
          ...
          <div style={{ maxWidth: '500px', margin: '0 auto' }}>
            <input 
              ...
              style={{ width: '100%', padding: '1rem 1.5rem', borderRadius: '30px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', color: 'white', outline: 'none' }}
            />
          </div>
        </div>
                ...
                <div key={post.id} style={{ background: 'var(--bg-card)', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--border-faint)', transition: '0.3s' }} className="blog-card">
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                      <Link to={`/category/${post.category}`} className="text-gold text-xs uppercase tracking-wider font-semibold" style={{ textDecoration: 'none' }}>{post.category}</Link>
